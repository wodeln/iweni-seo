<template>
  <div class="p-find-person">
    <header class="m-pos-f m-box m-aln-center m-justify-bet m-lim-width m-bb1 m-main m-head-top" style="padding: 0 10px;">
      ...
      <div class="m-box m-aln-center m-flex-grow0 m-flex-shrink0 m-justify-end m-location">
  ...
  methods: {
    ...
    onMoreClick () {
      ...
      actions.push({
        text: '只看男生',
        method: () => {
          this.$store.commit('UPDATE_USER_SEX', 0);
        },
      });
      actions.push({
        text: '只看女生',
        method: () => {
          this.$store.commit('UPDATE_USER_SEX', 2);
        },
      });
      actions.push({
        text: '查看全部',
        method: () => {
          this.$store.commit('UPDATE_USER_SEX', 1);
        },
      });
