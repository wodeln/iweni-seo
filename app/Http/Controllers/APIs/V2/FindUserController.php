    ...
    public function populars(Request $request, UserExtraModel $userExtra, ResponseContract $response)
    {
        ...
        $users = $userExtra
            ...
            ->whereExists(function ($query) {
                ...
                return $query->from('users')->whereRaw('users.id = user_extras.user_id')->where('deleted_at', null)->where('sex',$this->sex);
            })
            ->limit($limit)
            ->select('user_id')
            ->with([
    ...
    public function recommends(Request $request, UserRecommendedModel $userRecommended, ResponseContract $response)
    {
        ...
        ->whereExists(function ($query) {
            if($this->sex==1)
                ...
                return $query->from('users')->whereRaw('users.id = users_recommended.user_id')->where('deleted_at', null)->where('sex',$this->sex);
        })
        ->with(['user'])
        ->limit($limit)
        ->orderBy('id', 'desc')
